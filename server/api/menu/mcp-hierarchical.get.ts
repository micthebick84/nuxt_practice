...
export default defineEventHandler(async (event): Promise<MenuResponse> => {
  try {
    ...
    console.log('MCP 메뉴 조회 요청 (GET):', { userId, siteName, auth, menuAuthNo })

    // 실제 MCP 호출을 시도합니다
    try {
      // MCP 서버에 직접 요청 (GET 방식)
      ...
      console.log('=== MCP 응답 상세 ===')
      console.log('MCP 응답 전체:', mcpResponse)
      console.log('MCP 응답 데이터:', mcpResponseTyped?.data)
      console.log('========================')
      if (mcpResponseTyped && mcpResponseTyped.data) {
        const menuData = parseMCPMenuResponse(mcpResponseTyped.data)
        
        // 배열 크기로 성공 여부 판단
        const dataArraySize = menuData.length
        const isSuccess = dataArraySize > 0
        
        console.log('MCP 메뉴 데이터 배열 크기:', dataArraySize)
        console.log('배열 크기 기반 성공 여부:', isSuccess)
        
        if (isSuccess) {
          console.log('MCP 응답 성공 - 메뉴 데이터 반환 (배열 크기:', dataArraySize, ')')
          return {
            success: true,
            data: menuData,
            message: `MCP를 통해 메뉴 데이터를 성공적으로 가져왔습니다. (${dataArraySize}개 항목)`
          }
        }
      }
    } catch (mcpError) {
