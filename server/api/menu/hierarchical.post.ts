...
export default defineEventHandler(async (event): Promise<ApiMenuItem[]> => {
  try {
    // POST 요청의 body에서 파라미터 추출
    const body = await readBody(event)
    const userId = body.userId || 'admin'
    const siteName = body.siteName || 'Netis v6.6'
    const auth = body.auth || 'admin'
    const menuAuthNo = body.menuAuthNo || '1'

    // 8080 포트 백엔드 서버에서 계층형 메뉴 목록 가져오기
    console.log('백엔드 메뉴 조회 요청 (POST):', { userId, siteName, auth, menuAuthNo })

    try {
      // 8080 포트 백엔드 서버에 직접 요청 (POST 방식)
